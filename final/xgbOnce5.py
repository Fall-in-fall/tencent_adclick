...
def pre2(trainFeatures,trainLabels,testFeatures,testIns,preName,n,r,m):
    ...
    gbclf=xgb.XGBClassifier(n_estimators=n,learning_rate=r,max_depth=m,gamma=0,subsample=0.9,colsample_bytree=0.5
                            ,nthread=7)#verbose=0,max_leaf_nodes=8,loss="deviance"
...
if __name__=="__main__":
    ...
    fieldstr='userAllAndPosiConnRatio'
    ...
    testData='../data/temp/test_preAction_allIDField_4_addUserField.csv'
    trainFeatures,trainLabels,testFeatures,testIns=dataProcessing(dataPath,testData,field)

    pre2(trainFeatures,trainLabels,testFeatures,testIns,'xgb_{field}'.format(field=fieldstr),2750,0.08,5)
    pre2(trainFeatures,trainLabels,testFeatures,testIns,'xgb_{field}'.format(field=fieldstr),3200,0.08,5)
    pre2(trainFeatures,trainLabels,testFeatures,testIns,'xgb_{field}'.format(field=fieldstr),3000,0.08,5)
    pre2(trainFeatures,trainLabels,testFeatures,testIns,'xgb_{field}'.format(field=fieldstr),3500,0.08,5)
